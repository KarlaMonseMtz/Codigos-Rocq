...
También la extración para Ocaml.


Codigo inspirado en https://github.com/foreverbell/verified
       *)
...
(* Definición polimorfica de listas 
    casos recursivos de construccion de listas *)
...
tipo inductivo. 
Listas de numeros naturales ordenadas crecientemente.
Se puede entender 'ordenada' como una propiedad que decide 
si una lista esta ordenada o no: ordenada : list nat -> Prop *)
Inductive ordenada : list nat -> Set :=  (*Type*)
...
Proof.
intros x l O.
induction O. (* induccion sobre la propiedad de estar ordenada*)
- simpl. apply ord_single. (* l == NilN*)
- simpl insert. (* l == [x0] *)
  destruct (Nat.leb x x0) eqn:Hleb. 
...
-- apply Nat.leb_le in Hleb. apply ord_cons.
      ...
      * apply ord_cons.
        + assumption.
        + assumption.
-- apply Nat.leb_gt in Hleb. simpl. 
  destruct (x <=? y) eqn:Hleb'.
      * apply Nat.leb_le in Hleb'. apply ord_cons.
        + apply Nat.lt_le_incl. assumption.
        + apply ord_cons.
          ++ assumption.
     ...
     * apply Nat.leb_gt in Hleb'. apply ord_cons.
        ...
        + apply Nat.leb_gt in Hleb'.  
          assert (Heq: insert x (Cons y l) = Cons y (insert x l)).
            { simpl. rewrite Hleb'. reflexivity. } 
          rewrite Heq in IHO. assumption.
Qed.
...
(* Tipos nativos de OCaml *)
Extract Inductive bool => "bool" [ "true" "false" ].
Extract Inductive list => "list" [ "[]" "(::)" ].

(* Naturales como int de OCaml *)
Extract Inductive nat => "int"
  [ "0" "succ" ]
  "(fun fO fS n -> if n = 0 then fO () else fS (n-1))".


(* Extraccion de operaciones sobre listas *)
Extraction "listas.ml" length app rev.

(*Extraccion de insertion sort *)
Extraction "insertsort.ml" insert insertsort sort ordenada insert_ordenada insertsort_ordenada.
